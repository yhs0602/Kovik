...
fun makeMockedMethods(clazz: Class<*>) = clazz.methods.map {
    makeMockedMethod(clazz, it)
} + clazz.constructors.map {
    makeMockedConstructor(clazz, it)
}

fun makeMockedConstructor(clazz: Class<*>, it: Constructor<*>) = object : MockedMethod {
...
fun makeMockedMethod(clazz: Class<*>, method: Method) = object : MockedMethod {
    override fun execute(
        args: Array<out RegisterValue>,
        environment: Environment,
        code: CodeItem,
    ...
    override val classId: TypeId
        get() = TypeId(clazz.descriptorString())
    override val parameters: List<TypeId>
        get() = method.parameterTypes.map { TypeId(it.descriptorString()) }
    override val name: String
        get() = method.name
                ...
                is DictionaryBackedInstance -> {
                    ...
                    if (backingSuperClass != null) {
                        try {
                            ...
                            backingSuperClass.constructors.first {
...
class GeneralMockedClass(
    ...
    fun getMethods(): List<MockedMethod> = makeMockedMethods(clazz)
